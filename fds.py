...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, bucket_num=45, bucket_start=0, start_update=0, start_smooth=1,
    ...
    # def new_smooth(self, features, labels, epoch):
    #     # if epoch < self.start_smooth:
    #     #     return features
    ...
    # def new_version_smooth(self, features):
    ...
    #         features[i] = new_calibrate_mean_var(
    #                 features[i],
    #                 self.running_mean_last_epoch[int(idx)],
    #                 self.running_var_last_epoch[int(idx)],
    #                 self.smoothed_mean_last_epoch[int(idx)],
    #                 self.smoothed_var_last_epoch[int(idx)])
    #     return features

    # def new_version_smooth_01(self, features):
    #     self.running_mean_last_epoch[torch.mean(self.running_mean_last_epoch,dim = -1) == 0]  = self.smoothed_mean_last_epoch[torch.mean(self.running_mean_last_epoch,dim = -1) == 0] 
    #     for i in range(features.shape[0]):
    #         xxt = torch.matmul(F.normalize(self.running_mean_last_epoch), F.normalize(features[i].view(1,8192)).permute(1,0)).squeeze(-1)
    #         # dis = (self.running_mean_last_epoch - features[i].view(1,-1)).square().sum(dim = -1)
    ...
    # def new_version_smooth_02(self, features):
